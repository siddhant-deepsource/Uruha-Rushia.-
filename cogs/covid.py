...
class Coronavirus(commands.Cog):
    ...
    def __init__(self, client):
        self.client = client
        if not hasattr(self.client, 'corona'):
            self.diseaseapi = diseaseapi.Client()
            self.client.covid = self.diseaseapi.covid19
    ...
    async def _jhucsse(self, country, province):
        ...
        data = await self.client.covid.jhucsse() #get all data from the JHU CSSE
    ...
    async def coronavirus(self, ctx, country=None, *, province=None):
        ...
        if not country:
            data = await self.client.covid.all(allow_none=True)

        elif province:
            if (country.lower() == "us" or country.lower() == "usa"):
                data = await self.client.covid.state(province, allow_none=True)
            ...
            data = await self.client.covid.country(country, allow_none=True)
        ...
        if isinstance(data, diseaseapi.covidstatistics.Global):
            ...
            yesterdays_data = await self.client.covid.country(country, yesterday=True, allow_none=True)
            generate_country_embed(embed, data, yesterdays_data)

        elif isinstance(data, diseaseapi.covidstatistics.State):
            yesterdays_data = await self.client.covid.state(province, yesterday=True, allow_none=True)
    ...
    async def coronavirushistory(self, ctx, country="all", *, province=None):
        ...
        if province:
            ...
            data = await self.client.covid.province_history(country, province, 15) #get the history for the given country with province
            
        else:
            data = await self.client.covid.country_history(country, 15) #if no province given, get the data for whole country
    ...
    async def state_historical(self, ctx, state):
        ...
        data = await self.client.covid.nyt_state(state)
        data = data[-15:]
    ...
    async def coronavirusleaderboard(self, ctx, sort="cases"):
        ...
        data = await self.client.covid.all_countries(sort=sort)
